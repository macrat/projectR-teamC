...
class Window:
	...
	def draw_bar(self, left: float, top: float, right: float, bottom: float, percent: float, horizontal: bool) -> None:
		assert 0 <= left <= 1 and 0 <= top <= 1 and 0 <= right <= 1 and 0 <= bottom <= 1
		assert -1 <= percent <= 1
	...
	def draw_text(self, x: float, y: float, text: str, color: typing.Tuple[int] = (255, 255, 255)) -> None:
		assert 0 <= x <= 1 and 0 <= y <= 1

		img = self.font.render(text, 1, color)
		self.screen.blit(img, (x * self.width - img.get_width()/2, y * self.height - img.get_height()/2))
	...
	def update(self) -> None:
		...
		self.draw_text(0.1, 0.05, '{left: 5.0%} {right: 5.0%}'.format(**inp['body']))
